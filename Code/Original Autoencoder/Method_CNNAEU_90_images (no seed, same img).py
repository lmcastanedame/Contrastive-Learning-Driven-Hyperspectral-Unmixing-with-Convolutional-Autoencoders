...
def training_input_fn_tf(hsi, patch_size, patch_number, alpha_value=None):
    ...
    if alpha_value is not None:
        alphas = tf.fill((patches.shape[0],), alpha_value)
...
def SAD(y_true, y_pred):
    """
    Vectorized Spectral Angle Distance (SAD) loss function.
    """
    y_true = tf.math.l2_normalize(y_true, axis=-1)
    y_pred = tf.math.l2_normalize(y_pred, axis=-1)
    dot_product = tf.reduce_sum(y_true * y_pred, axis=-1)
    angles = tf.math.acos(tf.clip_by_value(dot_product, -1.0, 1.0))
...
def train_chunked(model, dataset, alphas, params):
    ...
    @tf.function
    def train_step(patches, alpha):
        """
        A single training step wrapped in a tf.function for performance.
        """
    ...
    for epoch in range(params['epochs']):
        ...
        total_loss = 0
        num_batches = 0
        step = 0  # To manage gradient accumulation
        ...
        for start_idx in range(0, len(dataset), chunk_size):
            ...
            dataset_chunk = (
                tf.data.Dataset.from_tensor_slices((chunk_patches, chunk_alphas))
                .shuffle(buffer_size=chunk_size, reshuffle_each_iteration=False)
                .batch(batch_size)
            ...
            for patches, alpha in dataset_chunk:
                ...
                gradients, loss = train_step(patches, alpha)
            ...
            # Memory cleanup
            del chunk_patches, chunk_alphas, dataset_chunk
            gc.collect()
...
if __name__ == "__main__":
    ...
    num_augmentations = 90  # Number of augmentations
    num_runs = 25  # Number of independent runs
    results_folders = {
        './90 images/Results (25 runs, no seed, same images)/Blur (sigma = [0.1, 2.0])': {
            'p1': 0, 'p2': 0, 'p3': 0, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Blur (sigma = [0.1, 1.0])': {
            'p1': 0, 'p2': 0, 'p3': 0, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 1.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Crop (95)': {
            'p1': 1, 'p2': 0, 'p3': 0, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Crop (75)': {
            'p1': 1, 'p2': 0, 'p3': 0, 'p4': 0, 'flip' : 0, 'crop' : 0.75, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Crop (50)': {
            'p1': 1, 'p2': 0, 'p3': 0, 'p4': 0, 'flip' : 0, 'crop' : 0.5, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Vertical Flip': {
            'p1': 0, 'p2': 1, 'p3': 0, 'p4': 0, 'flip' : 1, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Horizontal Flip': {
            'p1': 0, 'p2': 1, 'p3': 0, 'p4': 0, 'flip' : 2, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Mix Flip': {
            'p1': 0, 'p2': 1, 'p3': 0, 'p4': 0, 'flip' : 3, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Random Flip': {
            'p1': 0, 'p2': 1, 'p3': 0, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
            },
        './90 images/Results (25 runs, no seed, same images)/Jitter': {
            'p1': 0, 'p2': 0, 'p3': 1, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
            },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Blur': {
        #     'p1': 1, 'p2': 0, 'p3': 0, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Flip': {
        #     'p1': 1, 'p2': 1, 'p3': 0, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Flip + Blur': {
        #     'p1': 1, 'p2': 1, 'p3': 0, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (1, 1)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Jitter': {
        #     'p1': 1, 'p2': 0, 'p3': 1, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Jitter + Blur': {
        #     'p1': 1, 'p2': 0, 'p3': 1, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Crop + Jitter + Flip': {
        #     'p1': 1, 'p2': 1, 'p3': 1, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Jitter + Flip': {
        #     'p1': 0, 'p2': 1, 'p3': 1, 'p4': 0, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
        #     },
        # './90 images/Results (25 runs, no seed, same images)/Jitter + Flip + Blur': {
        #     'p1': 0, 'p2': 1, 'p3': 1, 'p4': 1, 'flip' : 0, 'crop' : 0.95, 'sigma' : 2.0, 'alpha_range' : (0.8, 1.2)
        #     },
    }
    ...
    for dataset in datasets:
        ...
        # Pre-generate augmented images and alphas once for each folder
        pre_generated_data = {}  # To store pre-generated augmented patches and alphas for each folder
        ...
        for folder, params_dict in results_folders.items():
            ...
            if folder not in pre_generated_data:
                print(f"Pre-generating augmented data for folder: {folder}", flush=True)

                augmented_patches = []
                augmented_alphas = []

                def augment_and_extract(idx):
                    """
                    Function to augment the data and extract patches, used for parallelization.
                    """
                ...
                with ThreadPoolExecutor(max_workers=4) as executor:  # Adjust workers based on system capacity
                    results = list(executor.map(augment_and_extract, range(num_augmentations)))

                # Aggregate results
                for patches, alphas in results:
                ...
                pre_generated_data[folder] = (augmented_patches, augmented_alphas)

                # Trigger garbage collection
                gc.collect()

            # Retrieve pre-generated data
            augmented_patches, augmented_alphas = pre_generated_data[folder]
            ...
            for run in range(num_runs):
                ...
                # Trigger garbage collection after each run
                gc.collect()
