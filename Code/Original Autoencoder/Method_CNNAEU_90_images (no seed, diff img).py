...
if __name__ == "__main__":
    ...
    # Define augmentation and run settings
        ...
        for folder, params_dict in results_folders.items():
            ...
            for run in range(num_runs):
                ...
                def augment_and_extract(idx):
                    """
                    Function to augment the data and extract patches.
                    """
                    try:
                        ...
                        augmented_image = apply_augmentation(
                            data,
                            p1=params_dict['p1'], p2=params_dict['p2'],
                            p3=params_dict['p3'], p4=params_dict['p4'],
                            alpha=alpha_value
                        )

                        # Validate shape
                        if augmented_image.shape[0] == 0 or augmented_image.shape[1] == 0:
                            raise ValueError(f"Invalid augmented image shape: {augmented_image.shape}")
                ...
                # Cleanup memory
                print(f"Results saved for run {run + 1}. Cleaning up memory...", flush=True)
